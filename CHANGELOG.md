...
- **A display that reconnects while the session is locked stays dark.** A reset ESP32 or a wedged
  Stream Deck is reopened, and the fresh device was never told about the lock, so it sat lit until
  the next unlock. It is sent the blank again now, after its brightness.
