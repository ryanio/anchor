...
| ESP32 pulse display | **Running on hardware, but not yet on the glass.** Flashed to an ESP32-S3 N16R8 (16MB flash, 8MB PSRAM); the board announces itself over USB CDC and answers frames. Measured end to end: a full frame is 23,219 bytes in 60 messages and lands in ~105ms; an unchanged frame costs zero bytes. **The board does have a display** — an earlier note here said otherwise, on the strength of an I2C scan pointed at the wrong pins. The real bus is SDA=15/SCL=14 and carries a CST820 touch controller, an AXP2101, a QMI8658, an ES8311 and an IO expander, and GPIO 13 carries a ~58Hz tearing-effect signal, which is a panel refreshing. Its QSPI pin map is still unknown, so the frame is not yet visible. See `devices-esp32.md`. |
