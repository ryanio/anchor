...
<!-- Adapted from openclaw/openclaw .agents/skills/test-audit (MIT, Copyright (c) 2026 OpenClaw Foundation). -->
...
A cleanup with no target stops far too early. Give every sweep a quantified
goal, such as "remove 20% of the least useful tests while keeping line coverage
of the owning production code within 2 points of baseline". Measure coverage
before the first edit and after the last, with the same command, and report
both numbers. The goal is a ceiling on ambition, not a quota: never convert an
uncertain candidate into a deletion to reach it, and stop short with the reason
when the ledger runs out of honest `D` and `C` marks.
...
Never edit source or tests while the test runner is running in the checkout.

Each workspace keeps its own `node_modules`: run `npm ci` at the root and in
`service`, `executor` and `devices` first. There is no root `test` script.

1. Run the smallest owner and sibling tests: `node --test <file>` from the
   owning workspace, or the workspace suite: `npm test --prefix service`
   (likewise `executor`, `devices`, `widget`) and `node --test scripts/`.
   The devices paint tests need a working SVG rasteriser (ImageMagick plus
   `rsvg-convert`); without one they fail or skip, so check the pass count.
   `devices/src/*.test.ts` also compiles and runs the C++ tests in
   `devices/firmware/common/` with the host `c++`.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract, such as `node scripts/device.ts sim
   <target>` for firmware.
3. Measure coverage with the same command before and after: in each workspace,
   `node --test --experimental-test-coverage --test-coverage-exclude='**/*.test.*'
   --test-coverage-exclude='**/node_modules/**' --test-coverage-exclude='**/test/**'
   --test-reporter=lcov --test-reporter-destination=<ws>.lcov <the suite's globs>`,
   then sum `DA:` lines over the production files. Node only counts files a
   test loads, so compare per file and flag any file that drops out. C++ has
   no coverage measurement here.
4. Run `npm run format`, then `git diff --check`.
5. Run the repository verify gate, which is the CI job in
   `.github/workflows/ci.yml`: `npm run lint`, `node scripts/check-versions.ts`,
   `node scripts/check-contrast.ts`, `npx --prefix <ws> tsc --noEmit --project
   <ws>` for `service`, `executor` and `devices`, and every suite from step 1.
6. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
7. After final audit edits, review the diff (`/code-review`) before landing.

## Landing and continuation

Follow the repository's `AGENTS.md` for where work lands; by default that is a
direct push to the default branch. Stage with explicit pathspecs and commit
with the pathspec on the commit (`git commit -F - -- <paths>`). Land one
coherent owner-boundary batch per commit; after landing, refresh from current
